    ...
    class WOVCCApi {
        ...
        renderFixtures(fixtures, container, limit = null) {
            ...
            displayFixtures.forEach((fixture, index) => {
                ...
                html += `<a href="${matchUrl}" target="_blank" class="fixture-card-link fixture-item" style="text-decoration:none;color:inherit;display:block;"><div class="fixture-card"><div style="display:flex;justify-content:space-between;align-items:start;gap:20px;"><div style="flex:1;"><div style="font-weight:600;color:var(--primary-color);margin-bottom:8px;font-size:0.95rem;">${fixture.team_name_scraping||'Team'}</div><div style="color:var(--text-dark);margin-bottom:10px;font-size:1.05rem;font-weight:500;"><div style="margin-bottom:4px;"><span style="font-size:0.85rem;color:var(--text-light);font-weight:400;">Home: </span>${fixture.home_team}</div><div><span style="font-size:0.85rem;color:var(--text-light);font-weight:400;">Away: </span>${fixture.away_team}</div></div><div style="display:flex;gap:15px;flex-wrap:wrap;font-size:0.85rem;color:var(--text-light);">${timeText?`<div><svg style="width:14px;height:14px;display:inline-block;margin-right:4px;vertical-align:middle;" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path></svg>${timeText}</div>`:''}${locationText?`<div><svg style="width:14px;height:14px;display:inline-block;margin-right:4px;vertical-align:middle;" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg>${locationText}</div>`:''}</div></div><div style="text-align:right;font-size:0.9rem;color:var(--text-light);white-space:nowrap;flex-shrink:0;"><div style="font-weight:600;color:var(--primary-color);">${fixture.date_str||fixture.date_iso||''}</div></div></div></div></a>`;
            });
            container.innerHTML = html;
        }
