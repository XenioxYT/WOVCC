(function() {
    ...
    const API_CONFIG = {
        baseURL: window.APP_CONFIG ? window.APP_CONFIG.apiBase : (
    ...
    class WOVCCApi {
        ...
        renderFixtures(fixtures, container, limit = null) {
            ...
            displayFixtures.forEach((fixture, index) => {
                ...
                // Render form guides if available (shows recent form going into the match)
                let formGuideHtml = '';
                if ((fixture.form_guide_home && fixture.form_guide_home.length > 0) || 
                    (fixture.form_guide_away && fixture.form_guide_away.length > 0)) {
                    formGuideHtml = '<div style="display:flex;flex-direction:column;gap:6px;margin-top:10px;font-size:0.8rem;">';
                    
                    if (fixture.form_guide_home && fixture.form_guide_home.length > 0) {
                        formGuideHtml += '<div style="display:flex;align-items:center;gap:4px;"><span style="color:var(--text-light);min-width:50px;">Home:</span>';
                        fixture.form_guide_home.forEach(function(f) {
                            const fUpper = f.toUpperCase();
                            let formBg = '#e9ecef';
                            let formColor = '#495057';
                            if (fUpper === 'W') { formBg = '#28a745'; formColor = '#fff'; }
                            else if (fUpper === 'L') { formBg = '#dc3545'; formColor = '#fff'; }
                            else if (fUpper === 'D' || fUpper === 'T') { formBg = '#ffc107'; formColor = '#212529'; }
                            formGuideHtml += `<span style="display:inline-block;width:18px;height:18px;line-height:18px;text-align:center;font-size:0.65rem;font-weight:700;border-radius:3px;background:${formBg};color:${formColor};">${fUpper}</span>`;
                        });
                        formGuideHtml += '</div>';
                    }
                    
                    if (fixture.form_guide_away && fixture.form_guide_away.length > 0) {
                        formGuideHtml += '<div style="display:flex;align-items:center;gap:4px;"><span style="color:var(--text-light);min-width:50px;">Away:</span>';
                        fixture.form_guide_away.forEach(function(f) {
                            const fUpper = f.toUpperCase();
                            let formBg = '#e9ecef';
                            let formColor = '#495057';
                            if (fUpper === 'W') { formBg = '#28a745'; formColor = '#fff'; }
        ...
        renderResults(results, container, limit = null) {
            ...
            displayResults.forEach((result, index) => {
                // Determine result class and label including new states
                ...
                } else if (result.is_loss) {
                    resultClass = 'loss';
                    resultLabel = 'Lost';
                    badgeStyle = 'background:#f8d7da;color:#721c24;';
                } else if (result.is_abandoned) {
                    resultClass = 'abandoned';
                    resultLabel = 'Abd';
                    badgeStyle = 'background:#e9ecef;color:#495057;';
                } else if (result.is_tied) {
                    resultClass = 'draw';
                    resultLabel = 'Tied';
                    badgeStyle = 'background:#fff3cd;color:#856404;';
                }

                // Determine opponent logo (show opponent's badge based on which team is Wickersley)
                const isHome = result.home_team && result.home_team.toLowerCase().includes('wickersley');
                const opponentLogo = isHome ? result.away_logo_url : result.home_logo_url;
                const logoHtml = opponentLogo 
                    ? `<img src="${opponentLogo}" class="opponent-logo" style="width:24px;height:24px;border-radius:50%;margin-right:8px;object-fit:contain;background:#fff;border:1px solid var(--border-color);" alt="Opponent">` 
                    : '';

                // Render form guide if available
                let formGuideHtml = '';
                if (result.form_guide && result.form_guide.length > 0) {
                    formGuideHtml = '<div style="display:flex;gap:4px;margin-top:8px;align-items:center;"><span style="font-size:0.75rem;color:var(--text-light);margin-right:4px;">Form:</span>';
                    result.form_guide.forEach(function(f) {
                        const fUpper = f.toUpperCase();
                        let formBg = '#e9ecef';
                        let formColor = '#495057';
                        if (fUpper === 'W') { formBg = '#28a745'; formColor = '#fff'; }
                ...
                html += `<a href="${matchUrl}" target="_blank" class="result-card-link result-item" style="text-decoration:none;color:inherit;display:block;"><div class="result-card ${resultClass}"><div class="fixture-date-mobile" style="font-weight:600;color:var(--primary-color);margin-bottom:12px;font-size:0.9rem;">${result.date_str||result.date_iso||''}</div><div class="fixture-card-content"><div class="fixture-card-main"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">${logoHtml}<span style="font-weight:600;color:var(--primary-color);font-size:0.95rem;">${result.team_name_scraping||'Team'}</span><span style="font-size:0.8rem;font-weight:600;padding:3px 10px;border-radius:12px;${badgeStyle}">${resultLabel}</span></div><div style="color:var(--text-dark);margin-bottom:8px;font-size:1.05rem;font-weight:500;"><div style="margin-bottom:4px;"><span style="font-size:0.85rem;color:var(--text-light);font-weight:400;">Home: </span>${result.home_team} | ${result.home_score?`<strong>${result.home_score}</strong>`:''}</div><div><span style="font-size:0.85rem;color:var(--text-light);font-weight:400;">Away: </span>${result.away_team} | ${result.away_score?`<strong>${result.away_score}</strong>`:''}</div></div>${result.summary?`<div style="font-size:0.88rem;color:var(--text-light);line-height:1.4;margin-top:4px;">${result.summary}</div>`:''}${formGuideHtml}</div><div class="fixture-date-desktop" style="text-align:right;font-size:0.9rem;color:var(--text-light);white-space:nowrap;"><div style="font-weight:600;color:var(--primary-color);">${result.date_str||result.date_iso||''}</div></div></div></div></a>`;
            });
            container.innerHTML = html;
