...
def convert_to_webp(input_path, output_path, delete_original=True):
    ...
    try:
        ...
        if delete_original:
            ...
            logger.info(f"  Deleted original: {os.path.basename(input_path)}")
...
def main():
    ...
    logger.info("\n" + "="*60)
    logger.info("IMAGE CONVERSION TO WEBP (AUTO-CONFIRM)")
    logger.info("="*60)
    logger.info("\nThis script will:")
    ...
    logger.info("\n⚠️  Starting conversion automatically in 2 seconds...")
    import time
    time.sleep(2)
